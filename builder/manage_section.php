<?php 
  ...
  $pagetitle="Section";
  $sel= "SELECT `section`.*,`client`.`client_name`,`project`.`project_name` FROM `section` INNER JOIN `project` ON `section`.`project_id`=`project`.`id` INNER JOIN `client` ON `section`.`client_id`=`client`.`id`" ;

  if(isset($_REQUEST['client_id']) && $_REQUEST['client_id']!='')
  {
    $sel.=" WHERE `section`.`client_id`=".$_REQUEST['client_id'];
  }
  if(isset($_REQUEST['project_id']) && $_REQUEST['project_id']!='')
  {
    $sel.=" AND `section`.`project_id`=".$_REQUEST['project_id'];
  }
  $sel.=" ORDER BY `section`.`client_id` ASC,`section`.`project_id` ASC";
...
<?php if(isset($_GET["msg"])) { ?>
	...
	<?php
            ...
            elseif($_GET["msg"]==4)
                    echo "Section with this name is already Exist.";	
            elseif($_GET["msg"]==5)
                    echo "This Section is in use. You can not delete this Section.";
            elseif($_GET["msg"]==6)
                    echo "Section Status Updated Successfully.";
    ?>
        </div>
<?php } ?> 
      ...
      <?php 
                    if(($_REQUEST['client_id']!='' && $_REQUEST['project_id']!='') || $_REQUEST['client_id']!='')
                    {
...
?>
                  <tr style="text-align: left;">
                    ...
                    <td style="text-align: center;">
                      <div class="btn-group">
                        <a class="btn btn-info btn-sm" href="#" onclick="window.location.href='add_section.php?id=<?php echo $data->id; ?>&mode=edit'">
                          <i class="glyphicon glyphicon-pencil"></i>
                        </a>
                        <a class="btn btn-danger btn-sm" href="add_section.php?id=<?php echo $data->id; ?>&mode=delete" onclick="return deleteConfirm();">
...
?>
              ...
              <a href="add_section.php" role="button" class="btn btn-primary">Add New</a>
</form>
